...
// RGB Framebuffers
uint8_t frameBuffer[3*64]; //PROBAR 3*64
...
//uint32_t Color(uint8_t Color) {
//	Color = 255;
//  return newColor(Color,0,0);
//}

//void visColor(uint8_t *frameBuffer, uint32_t frameBufferSize, uint32_t effectLength)
//{
//	uint32_t i;
//	static uint8_t x = 0;
//
...
//	{
//		uint32_t color = 256 & 0xFF;
//
//		frameBuffer[i*3 + 0] = color & 0xFF;
//		frameBuffer[i*3 + 1] = 0 >> 8 & 0xFF;
//		frameBuffer[i*3 + 2] = 0 >> 16 & 0xFF;
//	}
//}
...
void visBarrido(uint8_t *frameBuffer, uint32_t frameBufferSize, uint32_t effectLength)
{
	uint32_t i;
	static uint8_t x = 0;
	...
	for( i = 0; i < frameBufferSize / 3; i++)
	{
		...
		frameBuffer[i*3 + 0] = color_r & 0xFF;
		frameBuffer[i*3 + 1] = color_g >> 8 & 0xFF;
		frameBuffer[i*3 + 2] = color_b >> 16 & 0xFF;
	}
}
...
void visHandle2()
{
	...
	if(HAL_GetTick() - timestamp > 10)
	{
		...
		visBarrido(frameBuffer, sizeof(frameBuffer),20);
